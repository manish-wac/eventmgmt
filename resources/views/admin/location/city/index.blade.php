						...
						<div class="col-12">
							...
							<div class="portlet">
								<div class="portlet-header portlet-header-bordered">
									<h3 class="portlet-title">CITYS LIST</h3>
									<div class="col-md-6 add-new">
										<a href="{{route('admin.location.city.add')}}"><i style="font-size: 3em;" class="fa fa-3x fa-plus-circle"></i></a>
									</div>
								</div>
								...
								<div class="portlet-body">
									...
									<!-- BEGIN Datatable -->
									<table id="js-city-list" class="table table-bordered table-striped table-hover">
										<thead>
											<tr>
                        						<th>ID</th>
                                                <th>City</th>
...
var  BASE_URL = "{{url('/')}}";
 $(function () {
	var table = $('#js-city-list').DataTable({
		processing: true,
		serverSide: true,
		ajax: "{{ route('admin.location.city.datatable') }}",
		columns: [
	...
	$(document).on('click', '.js-delete-state', function () {
		...
		Swal.fire({
			title: 'Are you sure, do you want to delete this city?',
			...
			if (result.value) {
				$.ajax({
					url: BASE_URL+'/admin/location/city/delete/'+_id ,
					type: "DELETE",
					dataType: "json",
					success: function(result) {
						...
						$("#js-city-list").DataTable().ajax.reload();
						setTimeout(function() {
							$('#common-alert').remove();
						}, 1000);
					},
