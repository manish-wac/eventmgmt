...
{{--										<a href="{{route('user.location.event.add')}}"><i style="font-size: 3em;" class="fa fa-3x fa-plus-circle"></i></a>--}}
									</div>
								</div>
...
var  BASE_URL = "{{url('/')}}";
 $(function () {
	var table = $('#js-event-list').DataTable({
		processing: true,
		serverSide: true,
		ajax: "{{ route('user.location.event.datatable') }}",
		columns: [
	...
	$(document).on('click', '.js-delete-state', function () {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: BASE_URL+'/user/location/event/delete/'+_id ,
					type: "DELETE",
					dataType: "json",
